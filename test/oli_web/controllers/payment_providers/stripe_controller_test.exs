defmodule OliWeb.PaymentProviders.StripeControllerTest do
  ...
  describe "user cannot access when is not logged in or is guest" do
    ...
    test "redirects to new session when accessing the show view as guest", %{
      conn: conn,
      section: section
    } do
  ...
  describe "init intent" do
    ...
    test "fails when stripe api call fails", %{
      ...
      assert response(conn, 500) =~
               "Could not create stripe intent"
    end
